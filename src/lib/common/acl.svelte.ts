...
export class ACLBuilder implements ACL {
	...
	private static normalizePrefix(
		name: string,
		type: PrefixType,
	): { prefixed: string; stripped: string } {
	...
	renameHost(nameOld: string, nameNew: string) {
		...
		this.acls.forEach((acl) => {
			...
			acl.dst = acl.dst.map((dst) =>
				ACLBuilder.getPolicyDstHost(dst) === nameOld
					? nameNew + ':' + ACLBuilder.getPolicyDstPorts(dst)
					: dst,
			);
		});
	}
	...
	renameTag(nameOld: string, nameNew: string) {
		...
		const { stripped: strippedOld, prefixed: prefixedOld } = ACLBuilder.normalizePrefix(
			nameOld,
			'tag',
		);
		...
		this.acls.forEach((acl) => {
			...
			acl.dst = acl.dst.map((dst) =>
				ACLBuilder.getPolicyDstHost(dst) === prefixedOld
					? prefixedNew + ':' + ACLBuilder.getPolicyDstPorts(dst)
					: dst,
			);
		});
		if (this.ssh !== undefined) {
...
export async function saveConfig(
	...
	loading?: { setLoadingTrue: () => void; setLoadingFalse: () => void },
) {
